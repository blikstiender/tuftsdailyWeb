...
//widget output
	function get_theme_tweets($username, $consumerkey, $consumerkeysecret, $accesstoken, $accesstokensecret) {
				...
				if(!empty($tp_twitter_plugin_tweets)){
						...
						foreach($tp_twitter_plugin_tweets as $tweet){								
							print '<li><i class="icon-twitter"></i><span>'.convert_links($tweet['text']).'</span><a class="created radius label blue" target="_blank" href="http://twitter.com/'.$username.'/statuses/'.$tweet['status_id'].'">'.relative_time($tweet['created_at']).'</a></li>';
