...
class td_page_generator {
    ...
    static function get_bbpress_breadcrumbs($page_title) {
        ...
        $breadcrumbs_array [] = array (
            'title_attribute' => '',
            'url' => esc_url(get_permalink( get_page_by_path( 'forums' ) )),
    ...
    $pagenavi_options = self::pagenavi_init();
        ...
        if(!is_admin() and is_category()) {
            ...
            //get the slider view option for this category
            // @todo not sure if this works with permalinks
            if (isset($wp_query->query['cat'])) {
                $disable_category_big_grid_hide_option = td_util::get_category_option($wp_query->query['cat'], 'tdc_slider');
            } else {
