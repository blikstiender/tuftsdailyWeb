    ...
    function update_shortcode(){
			...
			switch(name) {
				...
				case 'btn':
					...
					content = '[button link="'+link+'" color="'+color+'" icon="'+icon+'" size="'+size+'"]'+title+'[/button]';
				...
				case 'single_icon':
					...
					if(boxed) { var round = 'round="true"' } else { var round = '' }
					if(link) { var url = 'url="'+link+'"' } else { var url = '' }
					
					content = '[icon type="'+icon+'" size="'+size+'" '+url+' '+box+' '+round+']';	
				break;
